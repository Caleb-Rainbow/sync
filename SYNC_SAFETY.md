...
同步开关关闭的任务由 Worker 自行返回成功（附带 `KEY_SYNC_SKIPPED=true` 仅供宿主 UI 识别），协调器视为本轮已完成：只要全部任务成功，`syncDataTime` 照常推进。停用期间其他启用任务保持正常增量同步，不会从旧游标反复重复处理数据。

代价是任务重新启用后从当前游标开始增量同步，停用窗口内变更的数据不会自动回补；需要补齐时由宿主执行一次全量同步（覆盖本地模式或手动指定较早起点）。

`getAllTask()` 返回全部已注册任务（含停用项），由 Worker 自行按开关跳过；任务集合/同步选项在轮内变化不再冻结游标。仅当轮内账号或设备号发生变化时不推进本轮时间戳，成功更新器提交前再次核对身份。
...
测试覆盖部分获取/预处理失败、null 响应、删除失败、上传期间业务修改、附件保留与清理、钩子回调、同秒冲突、停用任务视为成功推进游标、轮内身份变化冻结游标、持久化失败重试、emoji/NUL 大消息，以及真实测试 WorkManager 的 KEEP、取消、配置变化、提交失败流程。JDK 25 的 Robolectric 导出参数已加入测试任务配置。
